...
int main(int argc, char **argv) {
  ...
  String loadProfilePath(
      "build/_deps/profile-data-src/CIGRE_MV_NoTap/load_profiles/");
#endif
  ...
  String simName = "UDP_CIGRE-MV-NoTap";
  ...
  for (size_t i = 0; i < sys.mNodes.size(); ++i) {
    auto n = sys.mNodes[i];
    bool last = (i == sys.mNodes.size() - 1);

    signalOutConfig += fmt::format(R"STRING(
        {{
          "name": "{}.V.mag",
          "type": "float"
        }},
  ...
  const std::string config = fmt::format(
    ...
    "in": {{
      ...
      signalOutConfig);
  DPsim::Logger::get("UDP_CIGRE_MV_PowerFlowTest_LoadProfiles")
